...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const activity = getActivityBySlug('curricular', slug);
  if (!activity) return {};
  return {
    title: `${activity.title} | Curricular Activities | SVCP`,
    description: activity.shortDescription,
  };
}

export default async function CurricularDetailPage({ params }: Props) {
  const { slug } = await params;
  const activity = getActivityBySlug('curricular', slug);
  if (!activity) return notFound();
  const related = getRelatedActivities('curricular', slug, 3);
  const localGalleryImages = getLocalActivityGalleryImages('curricular', slug);
    ...
    <>
      <HeroBanner
        ...
        breadcrumbs={[
          ...
          { label: 'Curricular & Co-curricular Activities', href: '/Activity/curricularActivities' },
          { label: activity.title },
        ]}
        imageSrc={getLocalActivityDetailHero('curricular', slug)}
        fallbackSrc={activity.image}
      />
      <ActivityDetailLayout
        activity={activity}
        relatedActivities={related}
        backHref="/Activity/curricularActivities"
        backLabel="Curricular & Co-curricular Activities"
