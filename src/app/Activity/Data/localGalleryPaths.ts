...
import { getCategoryFolderCandidates, getActivityFolderCandidates } from '@/lib/activityImageUtils';

export function getLocalActivityGalleryImages(categorySlug: string, activitySlug: string): string[] {
  const categoryFolders = getCategoryFolderCandidates(categorySlug);
  const folderCandidates = getActivityFolderCandidates(categorySlug, activitySlug);
  const gallerySubfolders = ['detail/gallery', 'detail/gallary', 'detail/gallry', 'listing/detail/gallery', 'gallery', 'gallary', 'gallry'];

  for (const catFolder of categoryFolders) {
    const baseCategoryDir = path.join(process.cwd(), 'public', 'activity-images', catFolder);
    ...
    for (const actFolder of folderCandidates) {
      for (const sub of gallerySubfolders) {
        ...
        if (fs.existsSync(galleryDir)) {
          ...
          if (files.length > 0) {
            return files.map((file) => `/activity-images/${catFolder}/${actFolder}/${sub}/${file}`);
          }
        }
      }
    }
